...
\end{equation}
\end{theorem}
In other words, every bandlimited \(L^2\) function can be perfectly reconstructed from its infinitely but countable many samples, if the sampling rate is high enough! Holy Shit!\\
...
If we consider now a sound recording as a function $f$, $f$ is obviously compactly supported (there is a strict beginning and end). Consequently the fourier transform $\hat{f}$ can't be compactly supported ($f$ is not $\omega$-bandlimited). The manner of functioning of our ear allows us to ignore frequencies above 20 kHz and below 20 Hz. Although the reverse fourier transformation $\FF^{-1}(\hat{f}|_{Q})$ does not coincide with $\FF^{-1}(\hat{f})$, for our ear it is of no importance. So by Theorem \ref{th:interpolation}, we can reconstruct such prepared signals with arbitrary precision.\\\\
