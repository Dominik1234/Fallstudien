...
Wir brauchen zwei Beweise, einmal für $ \beta = \alpha \gamma $ und einmal für $ \gamma= \sum \beta \gamma $..\\
1.:  $ \alpha \gamma_{l}= \sum_{p \in C_l} \alpha^{l+1-r} $

Meine Ideen: Im Grunde ist das deshalb dasselbe wie $ \beta $, nur werden hier statt der r  Knoten aus M die l+1-r aus N betrachtet. Man kann für jeden Catalanpfad mit $  r_{i} $Flachstücken auch einen mit $  l+1-r_{i}$ finden, aber das ist leider nicht bijektiv. Wenn man wieder überlegt, wie viele Catalanpfade es gibt etc, ist irgendwann zu zeigen, dass $\sum_{r=1}^{l} \begin{pmatrix} 2l-2\\2r-2\end{pmatrix}  \left( C_{l-r} \alpha^r - C_{r-1} \alpha^{l+1-r} \right)  =0  $ ist. Bring das nur leider nicht raus.\\
