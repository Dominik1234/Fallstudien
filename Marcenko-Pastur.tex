...
Durch die Substitution $r := m+1-u-t$ und die Anwendung von Formel (\ref{eq:beta_l}) ergibt sich folgendes:
...
=& R_{m+2} + \alpha^{-2-m/2}(1+\alpha)\sum_{u=0}^{m+1} \binom{m+1}{u}(-\alpha-1)^u\beta_{m+2-u} \\
&- \alpha^{-2-m/2}(1+\alpha)\sum_{u=0}^{m+1}\binom{m+1}{u}(-\alpha-1)^u\beta_{m+2-u} \\
=& R_{m+2}
\end{align*}
...
\end{align}
(Der Schritt auf \(R_n\) stimmt leider nicht, da fehlt der Vorfaktor \(\alpha^{-1-n/2}\)! Mit dem gehts aber nicht mehr auf oder?)\\
